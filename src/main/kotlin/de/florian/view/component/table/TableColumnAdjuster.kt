...
/**
 *  Table Column Adjuster by Rob Camick ported to Kotlin
 ...
 */
class TableColumnAdjuster @JvmOverloads constructor(private val table: JTable, private val spacing: Int = 6) : PropertyChangeListener, TableModelListener {
    ...
    private fun updateTableColumn(column: Int, width: Int) {
        ...
        if (isOnlyAdjustLarger) {
            width = Math.max(width, tableColumn.preferredWidth)
        }
	 ...
	 */
    internal inner class ColumnAction(private val isSelectedColumn: Boolean, private val isAdjust: Boolean) : AbstractAction() {
        override fun actionPerformed(e: ActionEvent) {
            //  Handle selected column(s) width change actions
            if (isSelectedColumn) {
                val columns = table.selectedColumns
                for (i in columns.indices) {
                    if (isAdjust) adjustColumn(columns[i]) else restoreColumn(columns[i])
                }
            } else {
    ...
    /*
	 *  Toggle properties of the de.florian.view.component.table.TableColumnAdjuster so the user can
	 *  customize the functionality to their preferences
	 */
    internal inner class ToggleAction(private val isToggleDynamic: Boolean, private val isToggleLarger: Boolean) : AbstractAction() {
        override fun actionPerformed(e: ActionEvent) {
            if (isToggleDynamic) {
